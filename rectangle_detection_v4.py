...
green_lower = np.array([50, 50, 150])
...
dark_green_lower = np.array([25, 52, 72])
dark_green_upper = np.array([85, 200, 150])
light_blue_lower = np.array([100, 50, 120])
...
def detect_dots(frame):
    ...
    dot_lower = np.array([90, 50, 50])
    dot_upper = np.array([130, 255, 255])
...
while True:
    ...
    green_centers, green_axes = detect_largest_rectangles_with_axes(green_mask, frame, (0, 255, 0))
    yellow_centers, yellow_axes = detect_largest_rectangles_with_axes(yellow_mask, frame, (0, 255, 255))

    light_blue_centers, light_blue_axes = detect_largest_rectangles_with_axes(light_blue_mask, frame, (173, 216, 230))  # Light blue
    ...
    if len(light_blue_centers) == 2:
        light_blue_gradient = calculate_gradient(light_blue_centers[0], light_blue_centers[1])
        if light_blue_gradient is not None:
            gradients.append(light_blue_gradient)
    ...
    if gradients:
        ...
        if filtered_gradients:
            ...
            if len(yellow_centers) == 2:
                ...
                if yellow_axes[0] > yellow_axes[1]:
                    center1, center2 = np.array(yellow_centers[0]), np.array(yellow_centers[1])
                else:
                    center1, center2 = np.array(yellow_centers[1]), np.array(yellow_centers[0])

                x1, y1 = center1
                x2, y2 = center2
                y2 = int(y1 + avg_gradient * (x2 - x1))
                cv2.line(frame, center1, (x2, y2), (0, 255, 255), 2)

                # List of letters to display at equidistant points
                letters = ["Y1", "L1", "G1", "B1", "N1", "P1", "R2", "O2", "Y2"]
                extra_letters_left = ["R1", "O1"] # Reverse order from Y1
                extra_letters_right = ["L2", "G2", "B2", "N2"]
                # Draw the letters along the line between the red centers
                letter_positions = draw_letters_on_line(frame, center1, center2, letters, extra_letters_left, extra_letters_right)
            ...
            if len(green_centers) == 2:
                ...
                y2 = int(y1 + avg_gradient * (x2 - x1))
                cv2.line(frame, green_centers[0], (x2, y2), (0, 255, 0), 2)
            ...
            if len(light_blue_centers) == 2:
                x1, y1 = light_blue_centers[0]
                x2, y2 = light_blue_centers[1]
                y2 = int(y1 + avg_gradient * (x2 - x1))
                cv2.line(frame, light_blue_centers[0], (x2, y2), (255, 178, 102), 2)  # Light blue color in BGR
    ...
    # cv2.imshow('Dark Green Mask', dark_green_mask)
    cv2.imshow('Light Blue Mask', light_blue_mask)
